...
describe('PresetService', () => {
  ...
  beforeEach(fakeAsync(() => {
    ...
    const req = httpMock.expectOne(`${dictApiEndpoint}`);
  ...
  describe('preset I/O', () => {
    ...
    it('should emit an event when presets are saved', done => {
      presetService.savePresetsStream.subscribe((value) => {
        expect(value).toEqual({type: 'testType', target: 'testTarget', presets: [{'testPresetKey': 'testPresetValue'}]});
        done();
      });
    ...
    it('should save to user dictionary when presets are saved', fakeAsync(() => {
      ...
      presetService.savePresets('testType', 'testTarget', [{'testPresetKey': 'testPresetValue'}]);
      let savePresetReq = httpMock.expectOne(`${dictApiEndpoint}?key=${(PresetService as any).DICT_PREFIX}-testType-testTarget`);
      ...
      tick();
      flush();
      expect(userDictionaryService.getUserDictionary()[`${(PresetService as any).DICT_PREFIX}-testType-testTarget`])
        .toEqual([{'testPresetKey': 'testPresetValue'}]);
    }));
    ...
    it('should get user presets from the user dictionary', fakeAsync(() => {
      ...
      const testDict: any = {};
      testDict[testPresetKey] = testPresets;
  ...
  describe('operator preset handling', () => {
    ...
    it('should not set operator properties if a non operator preset is applied', fakeAsync(() => {
      ...
      expect(workflowActionService.getTexeraGraph().getOperator(mockPresetEnabledPredicate.operatorID).operatorProperties)
        .toEqual({
          presetProperty: 'testPresetProperty',
          normalProperty: 'testNormalProperty'
        });
    ...
    it('should not set operator properties if an invalid operator preset is applied', fakeAsync(() => {
      expect(() => {
        presetService.applyPreset('operator', mockPresetEnabledPredicate.operatorID, {NotAPresetProperty: 'presetApplied'});
        flush();
      }).toThrow();
  ...
  describe('operator preset validation', () => {
    ...
    it('should reject an empty preset', () => {
      expect(presetService.isValidOperatorPreset(
        {},
        mockPresetEnabledPredicate.operatorID)).toBeFalse();
    });
    ...
    it('should accept a properly formatted preset', () => {
      expect(presetService.isValidOperatorPreset(
        ...
        mockPresetEnabledPredicate.operatorID)).toBeTrue();
    });
    ...
    it('should reject new presets if they already exist', () => {
      ...
      expect(presetService.isValidNewOperatorPreset(
        { presetProperty: 'presetHasBeenApplied' },
        mockPresetEnabledPredicate.operatorID)).toBeFalse();
    });
  ...
  describe('operator preset schema generation', () => {
    it('should generate a preset schema', () => {
      const operatorSchema = <CustomJSONSchema7> {
        ...
        properties: {
          presetProperty: {
            type: 'string', description: 'property that can be saved in presets', title: 'presetProperty', 'enable-presets': true
          },
    ...
    describe('getOperatorPreset - throw errors if invalid', () => {
      ...
      it('should throw an error if operator properties doesn\'t have all the preset properties', () => {
        expect(() => PresetService.getOperatorPreset(mockPresetEnabledSchema.jsonSchema,
          { wrongProperty: 'wrongpropertyPreset' })).toThrow();
      });

      it('should return a preset if operator properties has all the preset properties', () => {
        expect(PresetService.getOperatorPreset(mockPresetEnabledSchema.jsonSchema,
          { presetProperty: 'presetPropertyValue' })).toEqual({ presetProperty: 'presetPropertyValue' });
      });

      it('should return a preset if operator properties has a superset of preset properties', () => {
        expect(PresetService.getOperatorPreset(mockPresetEnabledSchema.jsonSchema,
          { presetProperty: 'presetPropertyValue', otherProperty: 'othervalue' })).toEqual({ presetProperty: 'presetPropertyValue' });
      });
    });
    ...
    describe('filterOperatorPresetProperties - doesn\'t guarantee preset is valid', () => {
      ...
      it('should filter out non preset properties', () => {
        expect(PresetService.filterOperatorPresetProperties(mockPresetEnabledSchema.jsonSchema,
          { wrongProperty: 'wrongpropertyPreset' })).toEqual({});
      });
