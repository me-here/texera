...
export class PropertyEditorComponent {
   ...
   *  such as debounce time and distance condition.
   *
  ...
  public createOutputFormChangeEventStream(
    ...
    return formChangeEvent
      ...
      .debounceTime(PropertyEditService.formInputDebounceTime)
  ...
  public handleRemoveBreakpoint() {
    this.propertyEditService.removeBreakpoint(this.currentLinkID);
    this.clearPropertyEditor();
  }

  public confirmChangeOperatorLogic() {
    this.setInteractivity(false);
    this.propertyEditService.confirmChangeOperatorLogic(this.currentOperatorID);
  }
  ...
  private handleDisableEditorInteractivity(): void {
    this.executeWorkflowService.getExecutionStateStream().subscribe(event => {
      if (this.currentOperatorID) {
        if (event.current.state in [ExecutionState.Completed, ExecutionState.Failed]) {
          this.setInteractivity(true);
        } else {
          this.setInteractivity(false);
        }
